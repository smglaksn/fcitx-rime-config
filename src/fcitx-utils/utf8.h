...
namespace utf8 {
 ...
 inline std::string::size_type lengthN(const std::string &s, size_t n) {
    return fcitx_utf8_strnlen(s.c_str(), n);
}

 inline bool validate(const std::string &s) { return fcitx_utf8_check_string(s.c_str()); }

 std::string UCS4ToUTF8(uint32_t code);

 inline uint32_t getCharValidated(const std::string &s, std::string::size_type off = 0,
                                                   int maxLen = 6) {
    if (off >= s.size()) {
        return 0;
    }
